...
// Wrapped in an IIFE so the bundle can be safely re-evaluated on every SPA
// mount of the Pro /compose route (Next.js client-side navigation destroys
// and recreates the DOM, but next/script won't re-run the module). The
// per-DOM guard below makes a second evaluation against the same DOM a no-op,
// so React StrictMode's double-invoke can't double-bind listeners.
(function () {
  const __form = document.getElementById("composerForm");
  if (!__form) return; // not on the composer page
  if (__form.dataset.composerInit === "1") return; // already wired this DOM
  __form.dataset.composerInit = "1";

// Storage indirection. In the hosted app this bundle wrote straight to
// localStorage; here the host page installs window.__pcStore — a synchronous
// Storage-alike backed by the encrypted vault — before this script loads, so
// drafts and preferences land in the vault instead of plain localStorage.
// Falls back to localStorage so the bundle still runs standalone (e.g. opened
...
const CONFIG = {
  ...
  maxDrafts: 50, // keep in sync with CAP_BY_KIND.compose in lib/drafts-server.ts
  maxExamples: 3,
};
...
const SUGGESTIONS = {
  ...
  bounds: {
    ...
    items: [
      ...
      // Reasoning-era bounds: current models run long, widen scope, and add
      // verification passes unprompted. These are the counterweights.
  ...
  context: {
    ...
    items: [
      ...
      "The input follows in the next message",
      "Current state: <where things stand now>",
      "Prior attempts that failed: <what and why>",
      "Definitions: <term> means <…>",
      "Hard constraints: <budget / deadline / platform>",
      "Source of truth: <link or 1-line>",
      "Out-of-band facts the model can't infer: <…>",
    ],
  },
  task: {
    mode: "append",
    items: [
      "Read the full input before responding",
      // Replaced "Work step by step; show intermediate reasoning": reasoning
      // models think by default, so asking for it is redundant — and asking a
      // model to reproduce its reasoning in the answer can trip a refusal.
      "State what 'done' looks like, then work — don't narrate the steps",
      "Group findings by severity (blocker / nit / praise)",
      "Cite the source for each claim (file:line, section, URL)",
      "Produce the deliverable, then a one-line summary of changes",
      "List anything that blocks completion as ASK items",
      "Return a single object — no prose outside it",
      "Compare options in a table, then recommend one",
    ],
  },
  success: {
    mode: "append",
    items: [
      ...
      "Under <N> words / <N> bullets total",
      "A junior could act on it with no follow-up questions",
      "No TODOs, placeholders, or '...' remain",
      "Matches the format shown in the example exactly",
      "Covers all items in the task list, none invented",
    ],
  },
  clarify: {
    mode: "append",
    items: [
      "Ask up to 2 questions if requirements are ambiguous before starting",
      "If a fact is missing, mark it ASK rather than guessing",
      "Confirm scope before making large or destructive changes",
      "State your assumptions explicitly if you proceed without asking",
      "Stop and ask if the input contradicts the context above",
    ],
  },
};
...
const MODELS = {
  // Current-generation Claude models take the bare first-party id on Vertex and
  // an `anthropic.`-prefixed id on Bedrock, so they omit `vertex` (it falls back
  // to `id`). The 4.x rows keep the snapshot forms they were published with.
  anthropic: [
...
// ---------- theme ----------
function initTheme() {
  const saved = PC_STORE.getItem(CONFIG.themeKey);
...
el.theme?.addEventListener("click", () => {
  ...
  PC_STORE.setItem(CONFIG.themeKey, next);
});

// ---------- format + model pickers ----------
function getFormat() {
  const saved = PC_STORE.getItem(CONFIG.formatKey);
...
function getSelectedModel() {
  ...
  const saved = PC_STORE.getItem(CONFIG.modelKey) || "";
...
function setFormat(fmt) {
  ...
  PC_STORE.setItem(CONFIG.formatKey, fmt);
...
function setModel(modelId) {
  ...
  PC_STORE.setItem(CONFIG.modelKey, `${target.provider}:${modelId}`);
  updatePreview();
}
...
function appendExamplePair(input = "", output = "") {
  ...
  pair.innerHTML = `
    ...
    <textarea class="example-input" name="example-input" aria-label="Example ${idx + 1} input" rows="2" spellcheck="true" placeholder="What the user/system gives the model"></textarea>
    <label class="example-sublabel">Output</label>
    <textarea class="example-output" name="example-output" aria-label="Example ${idx + 1} output" rows="2" spellcheck="true" placeholder="What the model should produce"></textarea>
  `;
...
function renderExampleChips() {
  ...
  select.name = "example-starter";
  select.setAttribute("aria-label", "Example starters");
...
function updatePreview() {
  ...
  const best = Number(PC_STORE.getItem(CONFIG.bestScoreKey) || 0);
  if (best > 0) {
  ...
  if (score > best && score >= 60) {
    PC_STORE.setItem(CONFIG.bestScoreKey, String(score));
...
// ---------- lifetime count ----------
function getLifetime() { return Number(PC_STORE.getItem(CONFIG.lifetimeKey) || 0); }
function bumpLifetime() {
  const next = getLifetime() + 1;
  PC_STORE.setItem(CONFIG.lifetimeKey, String(next));
  renderLifetime();
}
function renderLifetime() {
  if (!el.lifetime) return; // counter is omitted on the Pro /compose page
  const n = getLifetime();
...
function renderSuggestions() {
  for (const [slot, cfg] of Object.entries(SUGGESTIONS)) {
    ...
    const select = document.createElement("select");
    select.className = "suggestions-select";
    select.name = `suggestions-${slot}`;
    select.setAttribute("aria-label", `Idea starters for ${slot}`);
...
function applyChip(slot, text, mode) {
  ...
  if (mode === "replace") {
    ...
    const prefix = cur ? "\n" : "";
    const bullet = (slot === "bounds" || slot === "tools" || slot === "format" || slot === "context") ? "- " : "";
...
// ---------- drafts ----------
// ---------- drafts (local + optional Pro server sync) ----------
// When the Pro compose page marks the host with data-draft-sync="compose",
...
let syncActive = false;  // true once a server hydrate succeeds

function loadDrafts() {
  try { return JSON.parse(PC_STORE.getItem(CONFIG.draftsKey) || "[]"); } catch { return []; }
}
function saveDrafts(list) { PC_STORE.setItem(CONFIG.draftsKey, JSON.stringify(list)); }
function draftSnippet(state) {
  return (state.goal || state.role || state.task || "(empty draft)").slice(0, 60);
}

async function apiSaveDraft(state, title) {
  const res = await fetch(DRAFT_SYNC.endpoint, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ kind: DRAFT_SYNC.kind, payload: state, title: title || undefined }),
  });
  if (!res.ok) throw new Error("save failed");
  return (await res.json()).draft;
}

// The optional title the user typed for the draft about to be saved. Becomes
// the row label (server) / snippet (local); blank falls back to the auto snippet.
function readDraftTitle() {
  return (el.draftTitle && el.draftTitle.value.trim()) || "";
}
function clearDraftTitle() {
  if (el.draftTitle) el.draftTitle.value = "";
}

// Hydrate the drafts list. Pro: pull from the server; on any failure fall back
// to the device's localStorage so drafts never silently vanish.
async function hydrateDrafts() {
  if (DRAFT_SYNC) {
    try {
      const res = await fetch(
        `${DRAFT_SYNC.endpoint}?kind=${encodeURIComponent(DRAFT_SYNC.kind)}`,
        { cache: "no-store" },
      );
      if (res.ok) {
...
// One-time push of pre-existing local drafts up to the account so enabling
// sync doesn't appear to lose them. Idempotent via a migrated flag.
async function migrateLocalDrafts() {
  if (!syncActive || PC_STORE.getItem(DRAFTS_MIGRATED_KEY)) return;
  const local = loadDrafts();
  // Only auto-push when the account has no server drafts yet (a fresh switch to
  // sync). If it already has drafts, pushing local copies could evict real ones
  // (the server trims to the newest N) — leave those as local rows instead so
  // nothing is lost and nothing real is overwritten.
  if (local.length && serverDrafts.length === 0) {
    // Oldest first so the newest local draft stays newest on the server.
    const remaining = [];
    for (const d of [...local].reverse()) {
      try { serverDrafts.unshift(await apiSaveDraft(d.state, d.goalSnippet)); }
      catch { remaining.unshift(d); /* couldn't push — keep it on the device */ }
    }
    serverDrafts = serverDrafts.slice(0, CONFIG.maxDrafts);
    saveDrafts(remaining);  // only the drafts that failed to migrate stay local
  }
  PC_STORE.setItem(DRAFTS_MIGRATED_KEY, "1");
}

// Unified, newest-first rows: server drafts (when synced) plus any local drafts
// (the only rows in local mode, or sync-failed fallbacks in sync mode).
function draftRows() {
  const rows = [];
  if (syncActive) {
    for (const d of serverDrafts) {
      rows.push({ key: "S" + d.id, id: d.id, ts: new Date(d.createdAt).getTime(), snippet: d.label, state: d.payload, remote: true });
    }
  }
  for (const d of loadDrafts()) {
    rows.push({ key: "L" + d.ts, ts: d.ts, snippet: d.goalSnippet, state: d.state, remote: false });
  }
  rows.sort((a, b) => b.ts - a.ts);
  return rows.slice(0, CONFIG.maxDrafts);
}

async function saveCurrentAsDraft() {
  const state = readForm();
  if (!hasAnyContent(state)) return;
  const title = readDraftTitle();
  const snippet = title || draftSnippet(state);
  if (syncActive) {
    try {
      serverDrafts = [await apiSaveDraft(state, title), ...serverDrafts].slice(0, CONFIG.maxDrafts);
      renderDrafts();
      clearDraftTitle();
      flashBtn(el.saveBtn, "Saved ✓");
      return;
    } catch {
      // Surface the failure and keep the work locally instead of pretending.
      ...
      renderDrafts();
      clearDraftTitle();
      flashBtn(el.saveBtn, "Saved on device · sync failed");
      return;
    }
  }
  const list = loadDrafts();
...
async function removeDraft(row) {
  ...
  if (row.remote) {
    try {
      const res = await fetch(`${DRAFT_SYNC.endpoint}/${row.id}`, { method: "DELETE" });
      if (res.ok) serverDrafts = serverDrafts.filter((x) => x.id !== row.id);
    } catch { /* leave it; user can retry */ }
    renderDrafts();
    return;
  }
  saveDrafts(loadDrafts().filter((x) => x.ts !== row.ts));
  renderDrafts();
}

function renderDrafts() {
  // Pro compose omits the inline drafts card (drafts live on the /drafts page);
  // without a card to paint into there's nothing to render. Hydration/sync still
  // runs so saves reach the account — only the inline list is skipped.
  if (!el.draftsCard || !el.draftsList) return;
  const rows = draftRows();
  const header = document.querySelector(".drafts-header h3");
  if (header) header.textContent = syncActive ? "Saved to your account" : "Saved on this device";
  // Always rebuild from scratch and gate visibility on row count. (The card's
  // own `display:flex` overrides the [hidden] attribute, so we both empty the
  // list AND toggle `hidden` — backed by a .drafts-card[hidden] CSS rule — so
  // deleting the last draft actually clears the card without a refresh.)
  el.draftsList.innerHTML = "";
  // Card visibility tracks total drafts, not the filtered view — so a search
  // that matches nothing leaves the search box reachable to clear it.
  el.draftsCard.hidden = rows.length === 0;
  if (!rows.length) return;

  const query = ((el.draftsSearch && el.draftsSearch.value) || "").trim().toLowerCase();
  const visible = query
    ? rows.filter((r) => (r.snippet || "").toLowerCase().includes(query))
    : rows;
  if (!visible.length) {
    const li = document.createElement("li");
    li.className = "drafts-empty";
    li.textContent = `No drafts match “${query}”.`;
  ...
  for (const row of visible) {
    ...
    li.innerHTML = `
      <button class="draft-load" type="button" title="Load this draft">
        <span class="draft-when"></span>
        <span class="draft-snippet"></span>
      </button>
      <button class="draft-del" type="button" aria-label="Delete draft">Delete</button>
    `;
    li.querySelector(".draft-when").textContent = relTime(row.ts) + (row.remote ? "" : " · local");
    li.querySelector(".draft-snippet").textContent = row.snippet;
    li.querySelector(".draft-load").addEventListener("click", () => {
      writeForm(row.state);
      updatePreview();
      scrollToFirstField();
    });
    li.querySelector(".draft-del").addEventListener("click", () => removeDraft(row));
    el.draftsList.appendChild(li);
  }
}
...
el.newBtn.addEventListener("click", () => {
  ...
  clearForm();
  clearDraftTitle();
  PC_STORE.removeItem(CONFIG.currentKey);
  updatePreview();
  fields.role?.focus();
});
// Live-filter the saved-drafts list as the user types (purely client-side over
// the already-loaded rows).
if (el.draftsSearch) el.draftsSearch.addEventListener("input", renderDrafts);
el.clearDrafts && el.clearDrafts.addEventListener("click", async () => {
  if (!confirm("Delete all saved drafts?")) return;
  if (syncActive) {
    try {
      await fetch(`${DRAFT_SYNC.endpoint}?kind=${encodeURIComponent(DRAFT_SYNC.kind)}`, { method: "DELETE" });
      serverDrafts = [];
    } catch { /* ignore; clear local + re-render anyway */ }
  }
  PC_STORE.removeItem(CONFIG.draftsKey);
  renderDrafts();
});
...
// viewport under the sticky header — not the page top, which on mobile shows
// the hero/jump-start instead of the start of the new content.
...
// The /drafts library opens a draft by stashing its payload here and navigating
...
function restoreCurrent() {
  try {
    const raw = PC_STORE.getItem(CONFIG.currentKey);
...
// Precedence: a share-link hash, then a draft opened from the library, then the
// auto-saved current working state.
const loadedFromHash = loadFromHash();
if (!loadedFromHash && !loadFromPrefill()) restoreCurrent();
hydrateDrafts();
renderLifetime();
updatePreview();
...
function setMode(mode) {
  ...
  try { PC_STORE.setItem(MODE_KEY, mode); } catch {}
...
setMode(PC_STORE.getItem(MODE_KEY) || "single");
